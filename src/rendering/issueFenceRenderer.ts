...
import { COMMENT_REGEX, JIRA_KEY_REGEX } from '../settings/settings.models'
...
export const IssueFenceRenderer = async (
	source: string,
	el: HTMLElement,
		...
		if (issueKey) {
			...
			if (cachedIssue) {
				if (cachedIssue.isError) {
					renderedItems[issueKey] = RC.renderIssueError(
						issueKey,
						cachedIssue.data as string
					)
				} else {
					renderedItems[issueKey] = RC.renderIssue(
						SettingsData.account, cachedIssue.data as JiraIssue
					)
				}
			} else {
				...
				JiraClient.getIssue(issueKey)
					.then((newIssue) => {
						...
						renderedItems[issueKey] = RC.renderIssue(SettingsData.account, issue)
