...
export class JiraIssueSettingTab extends PluginSettingTab {
	...
	displaySearchColumnsSettings() {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(SettingsData.allFields).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addButton((button) =>
				button
					...
					.onClick(async (_value) => {
						// const { containerEl } = this
						// this.displaySearchColumnModifySettings()
						new ColumnSettings(containerEl, this.saveSettings.bind(this), this.display.bind(this)).render()
					})
			)
	}

	displaySearchColumnModifySettings() {
		const { containerEl } = this

		containerEl.empty()
		new Setting(containerEl).setName('Column configuration').setHeading()
		...
		SettingsData.searchColumns.forEach((column, index) => {
			const setting = new Setting(containerEl).then((setting) => {
