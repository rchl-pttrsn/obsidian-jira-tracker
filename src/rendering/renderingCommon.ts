...
export default {
	...
	renderLoadingItem(item: string, inline = false): HTMLElement {
		...
		createSpan({
			cls: 'spinner',
			parent: createSpan({ cls: `ji-tag`, parent: tagsRow }),
		})
	...
	renderIssue(account: JiraAccountSettings, issue: JiraIssue, compact = false): HTMLElement {
		const tagsRow = createDiv('ji-tags has-addons')
		createSpan({
			cls: `ji-tag ji-band`,
		...
		createEl('a', {
			cls: `ji-tag is-link no-wrap`,
			href: this.issueUrl(account, issue.key),
			title: this.issueUrl(account, issue.key),
			text: issue.key,
			parent: tagsRow,
		})
		if (!compact) {
		...
		const statusColor =
			JIRA_STATUS_COLOR_MAP_BY_NAME[issue.fields.status.name] ||
