	...
	if (SettingsData.inlinePrefix) {
		...
		while (
			...
			const container = createSpan({
				...
				attr: { 'data-issue-key': issueKey, 'data-compact': compact },
			})
...
function convertInlineIssuesUrlToTags(el: HTMLElement): void {
	if (SettingsData.inlineIssueUrlToTag) {
		...
		const issueUrlElements = el.querySelectorAll(
			`a.external-link[href^="${account.host}/browse/"]`
		)
		issueUrlElements.forEach((issueUrlElement: HTMLAnchorElement) => {
			const compact =
				issueUrlElement.previousSibling &&
				issueUrlElement.previousSibling.textContent.endsWith('-')
			const issueKey = issueUrlElement.href.replace(
				`${account.host}/browse/`,
				''
			)
			const container = createSpan({
...
export const InlineIssueRenderer = async (
	...
	const inlineIssueTags: NodeListOf<HTMLSpanElement> =
		el.querySelectorAll(`span.ji-inline-issue`)
		...
		if (cachedIssue) {
			if (cachedIssue.isError) {
				value.replaceChildren(
					RC.renderIssueError(issueKey, cachedIssue.data as string)
				)
			} else {
				value.replaceChildren(
