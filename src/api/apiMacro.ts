import { JiraAgileApi } from '../client/jira.models'
import API from './api'
const ms = require('ms')
...
export async function getActiveSprint(
	...
	if (boards.length > 0) {
		const sprints = await API.base.getSprints(boards[0].id, {
			state: [JiraAgileApi.SprintState.ACTIVE],
			limit: 1,
		})
		if (sprints.length > 0) {
			return sprints[0]
...
export async function getVelocity(
	projectKeyOrId: string,
	sprintId: number,
	storyPointFieldName: string = 'aggregatetimeoriginalestimate'
) {
